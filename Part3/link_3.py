...
class LinkFrame:
    ...
    def from_byte_S(cls, byte_S):
        type_S = byte_S[0: cls.type_S_length]
        if type_S == 'M':
            type_S = 'MPLS'
        elif type_S == 'N':
            type_S = 'Network'
        else:
            raise ('%s: unknown type_S field: %s' % (cls, type_S))
        data_S = byte_S[cls.type_S_length:]
        return cls(type_S, data_S)
...
class Link:
    ...
    # @param node_1: node from which data will be transferred
    # @param node_1_intf: number of the interface on that node
    # @param node_2: node to which data will be transferred
                ...
                if intf_a.next_avail_time <= time.time():
                    ...
                    print('%s: transmitting frame "%s" on %s %s -> %s %s \n'
                          ...
                          % (
                              ...
                              len(intf_a.out_queue)))

                    # iterates through a queue and prints the priorities
                    for element in intf_a.out_queue:
